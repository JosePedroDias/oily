...
-- https://rosettacode.org/wiki/Bitmap/Midpoint_circle_algorithm#Lua

local function circle(x, y, r, mm, mat)
...
local function band(x1, x2, y, mm, mat)
  if x1>x2 then
    x2, x1 = x1, x2
  end
  for x = x1, x2 do
...
local function filledCircle(x, y, r, mm, mat)
  local dx, dy, err = r, 0, 1 - r
  while dx >= dy do
    band(x-dx, x+dx, y+dy, mm, mat)
    band(x-dx, x+dx, y-dy, mm, mat)
    band(x-dy, x+dy, y+dx, mm, mat)
    band(x-dy, x+dy, y-dx, mm, mat)
...
M.filledCircle = filledCircle

-- https://rosettacode.org/wiki/Bitmap/Bresenham%27s_line_algorithm#Lua

local function line(x1, y1, x2, y2, mm, mat)
  ...
  while true do
    ...
    if (err < dy) then err, y1 = err + dx, y1 + sy end
  end
end
M.line = line
